...
public class Environment implements EnvironmentInterfaceStandard {
     ...
     * @param receivers The modules listed here will receive the event. If no
     *        modules are listed, the event will be thrown to all modules.
     */
    protected final void throwEvent(APLFunction e, String... receivers) {
        // Not implemented on the EIS -> 2APL side.
        // TODO: When EIS -> 2APL event handling is ready, this method should be
        // implemented.

        /*
         * // System.out.println("Sent event to " + receivers.length);
         * 
         * // Send event to all receivers, if no receivers specified, // send to
         * all modules in environment if (receivers.length > 0) { for (String
         * receiver : receivers) { APLModule module = modules.get(receiver); if
         * (module != null) module.notifyEEevent(e, getName()); //
         * System.out.println("Sent event to " + agent.getLocalName() ); } }
         * else { for (APLModule module : modules.values()) {
