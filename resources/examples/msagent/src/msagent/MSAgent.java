...
implements MSAgentListener
{
	 ...
	 * Note that the Microsoft speach drivers need to be installed.
...
class Env extends Thread
{
		...
		while (running)
		{
			...
			switch (state.number)
			{
				...
				case MSAgentActionEvent.SHOW:
					...
					result = Show(msAgentIDs.get(state.performer), false);
					break;
	...
	public void addMSAgentListener(MSAgent msAgent, MSAgentListener listener)
	{
	...
	public void removeMSAgentListener(MSAgent msAgent, MSAgentListener listener)
	{
		if (listeners.get(msAgent) == null)
			return;
